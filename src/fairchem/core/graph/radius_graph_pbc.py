...
def get_max_neighbors_mask(
    ...
    # Precompute index_neighbor_offset for efficiency
    index_neighbor_offset = torch.cumsum(num_neighbors, dim=0) - num_neighbors

    # NOTE: Avoid explicit Python for-loops and repeated memory allocations
    num_neighbors_cpu = (
        num_neighbors.cpu() if num_neighbors.device.type != "cpu" else num_neighbors
    )

    # index_sort_map assumes index to be sorted
    index_neighbor_offset_expand = torch.repeat_interleave(
        index_neighbor_offset, num_neighbors_cpu
    )
    index_sort_map = (
    ...
    # Instead of index_copy_, use scatter for in-place
    # Preallocate buffer with +inf for each possible neighbor slot
    ...
    distance_sort.scatter_(0, index_sort_map, atom_distance)
    ...
    if enforce_max_strictly:
        ...
        # Avoid index out of bound if number of neighbors is less than threshold
        effective_cutoff = torch.full(
            (num_atoms,),
            np.inf,
            device=device,
            dtype=distance_sort.dtype,
        )
        num_available = (distance_sort != np.inf).sum(dim=1)
        below_thresh = num_available > max_num_neighbors_threshold
        if below_thresh.any():
            # Only compute cutoff for those exceeding the threshold
            effective_cutoff[below_thresh] = (
                distance_sort[below_thresh, max_num_neighbors_threshold]
        ...
        max_num_included = num_included_per_atom.max()
...
def radius_graph_pbc(
    ...
    # Compute cell offsets only if periodic boundary in direction
    ...
    rep_axes = []
    if pbc[0]:
        cross = cross_a2a3 / cell_vol
        rep_axes.append(torch.ceil(radius * torch.norm(cross, p=2, dim=-1)))
    else:
        rep_axes.append(data.cell.new_zeros(1))
    if pbc[1]:
        cross = torch.cross(data.cell[:, 2], data.cell[:, 0], dim=-1) / cell_vol
        rep_axes.append(torch.ceil(radius * torch.norm(cross, p=2, dim=-1)))
    else:
        rep_axes.append(data.cell.new_zeros(1))
    if pbc[2]:
        cross = torch.cross(data.cell[:, 0], data.cell[:, 1], dim=-1) / cell_vol
        rep_axes.append(torch.ceil(radius * torch.norm(cross, p=2, dim=-1)))
    else:
        rep_axes.append(data.cell.new_zeros(1))

    max_rep = [axis.max() for axis in rep_axes]

    # Compute all cell shifts at once
    # Use torch.meshgrid for better memory locality, faster than torch.cartesian_prod for small dims
    rep_values = [
        torch.arange(
            -int(rep.item()), int(rep.item()) + 1, device=device, dtype=data.cell.dtype
        )
        for rep in max_rep
    ]
    grids = torch.meshgrid(*rep_values, indexing="ij")
    unit_cell = torch.stack([g.reshape(-1) for g in grids], dim=1)
    ...
    # Use broadcasting and reshape to avoid unnecessary expand/copy
    pos1 = pos1.view(-1, 3, 1)
    pos2 = pos2.view(-1, 3, 1) + pbc_offsets_per_atom

    pos1 = pos1.expand(-1, -1, num_cells)
    pos2 = pos2.expand(-1, -1, num_cells)
    index1 = index1.view(-1, 1).expand(-1, num_cells).reshape(-1)
    index2 = index2.view(-1, 1).expand(-1, num_cells).reshape(-1)

    # Compute squared distance and mask in-place
    ...
    # Use single indexing to mask everything in batch for better performance
    index1 = index1[mask]
    index2 = index2[mask]
    atom_distance_sqr = atom_distance_sqr[mask]
    unit_cell = unit_cell_per_atom.view(-1, 3)[mask]
