...
def get_max_neighbors_mask(
    ...
    num_atoms_int = int(num_atoms.item())  # Avoid future repeated .item() calls
    ...
    image_indptr = torch.empty(natoms.shape[0] + 1, device=device, dtype=torch.long)
    torch.cumsum(natoms, dim=0, out=image_indptr[1:])
    image_indptr[0] = 0
    ...
    # Main bottleneck optimization block

    # Get a simple index to fill distances, combining offset and in-neighbor position:
    index_neighbor_offset = torch.cumsum(num_neighbors, dim=0) - num_neighbors
    # For each edge (row in index), get the offset for that row's atom index
    # This is equal to, if i = edge, index[edge] is the atom.
    index_flat = index  # (num_edges,)
    # Get neighbor order within each atom: [0, 1, 2, ..., n_atom0-1, 0, 1, ...]
    neighbor_order = torch.arange(len(index), device=device) - torch.repeat_interleave(
        index_neighbor_offset, num_neighbors
    )
    index_sort_map = index_flat * max_num_neighbors + neighbor_order

    distance_sort = torch.full(
        (num_atoms_int * max_num_neighbors,),
        float("inf"),
        device=device,
        dtype=atom_distance.dtype,
    )
    distance_sort.index_copy_(0, index_sort_map, atom_distance)
    distance_sort = distance_sort.view(num_atoms_int, max_num_neighbors)
    ...
    if enforce_max_strictly:
        ...
        distance_sort[~is_included.T] = float("inf")

        num_included_per_atom = is_included.sum(dim=0)
        max_num_included = int(num_included_per_atom.max().item())
        distance_sort = distance_sort[:, :max_num_included]
        index_sort = index_sort[:, :max_num_included]

        # Recompute the number of neighbors (as per above, this is still correct)
        num_neighbors_thresholded = num_neighbors.clamp(max=num_included_per_atom)
        num_neighbors_image = sum_partitions(num_neighbors_thresholded, image_indptr)

    # Offset index_sort so that it indexes into index
    index_neighbor_offset_exp = index_neighbor_offset.view(-1, 1)
    # Expand offset for each neighbor slot
    index_sort = index_sort + index_neighbor_offset_exp

    # Remove unused pairs (distance == inf)
    mask_finite = torch.isfinite(distance_sort)
    index_sort = index_sort[mask_finite]
...
def radius_graph_pbc(
    ...
    index_offset_expand = torch.repeat_interleave(
        ...
        output_size=num_atoms_per_image_sqr.sum().item(),
    )  # output_size ops reduce dtype/size conversions
    ...
    num_atom_pairs = num_atoms_per_image_sqr.sum().item()
    index_sqr_offset = (
        torch.cumsum(num_atoms_per_image_sqr, dim=0) - num_atoms_per_image_sqr
    )
    index_sqr_offset = torch.repeat_interleave(
        index_sqr_offset, num_atoms_per_image_sqr, output_size=num_atom_pairs
    )
    ...
    # Use vectorization for PBC calculations; remove repeated torch.new_zeros calls for efficiency
    pbc_cfg = torch.tensor(pbc, dtype=torch.bool, device=device)
    rep = []
    for i, axes in enumerate([(1, 2), (2, 0), (0, 1)]):
        if pbc_cfg[i]:
            cross = torch.cross(data.cell[:, axes[0]], data.cell[:, axes[1]], dim=-1)
            inv_min = torch.norm(cross / cell_vol, p=2, dim=-1)
            rep.append(torch.ceil(radius * inv_min))
        else:
            rep.append(data.cell.new_zeros(1))

    max_rep = [r.max() for r in rep]

    cells_per_dim = [
        torch.arange(
            -rep_i.item(), rep_i.item() + 1, device=device, dtype=data.cell.dtype
        )
        for rep_i in max_rep
    ]
    unit_cell = torch.cartesian_prod(*cells_per_dim)
    num_cells = len(unit_cell)
    unit_cell_per_atom = unit_cell.view(1, num_cells, 3).expand(index2.size(0), -1, -1)

    unit_cell_t = unit_cell.t()
    unit_cell_batch = unit_cell_t.view(1, 3, num_cells).expand(batch_size, -1, -1)
    data_cell = data.cell.transpose(1, 2)
    pbc_offsets = torch.bmm(data_cell, unit_cell_batch)
    pbc_offsets_per_atom = torch.repeat_interleave(
        pbc_offsets, num_atoms_per_image_sqr, dim=0, output_size=index2.size(0)
    )

    # Expand and broadcast only where needed
    pos1 = pos1.unsqueeze(-1).expand(-1, 3, num_cells)
    pos2 = pos2.unsqueeze(-1).expand(-1, 3, num_cells)
    index1 = index1.unsqueeze(-1).expand(-1, num_cells).reshape(-1)
    index2 = index2.unsqueeze(-1).expand(-1, num_cells).reshape(-1)
    pos2 = pos2 + pbc_offsets_per_atom

    atom_distance_sqr = torch.sum((pos1 - pos2) ** 2, dim=1).reshape(-1)

    # Remove pairs that are too far apart or are the same atom
    ...
    index1 = index1[mask]
    index2 = index2[mask]
    unit_cell = unit_cell_per_atom.reshape(-1, 3)[mask]
    ...
    if not torch.all(mask_num_neighbors):
        ...
        unit_cell = unit_cell[mask_num_neighbors]
